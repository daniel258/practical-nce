# Run_D2_Sims_WithX.R
# Runs simulations for Design D2 (with V), with TWO MEASURED covariates included in ALL analyses.
...
# Measured covariates (Option A): affect A and Y, not N
ax_1 <- 0.15
ax_2 <- 0.15
bx_1 <- 0.2
bx_2 <- 0.2
...
out_dir <- "results/withX/D2"
design_label <- "D2_WithX"
...
tag <- sprintf("D2_withX_%s_n%d_it%d_seed%d", noise_dist, n_sample, n_iters, seed)
save_prefix <- file.path(out_dir, paste0(tag, "_", stamp))

# -------------------- grid --------------------
grid <- MakeGrid_D2_FixBiasFixRho_VaryF_WithX(
  rho_total = rho_total,
  f_vec = f_vec,
  a1 = a1,
  a2 = a2,

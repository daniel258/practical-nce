# Run_D0_Sims_WithX.R
# Run simulations for Design 0 (a2=c2=0),
...
# Grid choices (D0 varies a1 and c1)
...
out_dir <- "results/withX/D0"
design_label <- "D0_WithX"
...
tag <- sprintf("D0_withX_%s_n%d_it%d_seed%d", noise_dist, n_sample, n_iters, seed)
save_prefix <- file.path(out_dir, paste0(tag, "_", stamp))

# ---- build grid ----
grid <- MakeGrid_D0_WithX(
  a1 = a1_vec,
  c1 = c1_vec,
  b1 = b1,
  b2 = b2,

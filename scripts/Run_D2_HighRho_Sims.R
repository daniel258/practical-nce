...
tag <- sprintf("D2_highRho_unequalA2_%s_n%d_it%d_seed%d",
...
message(sprintf("[Run_D2_HighRho_Sims] Total runtime: %.1f sec (%.2f min)",
                elapsed_sec, elapsed_sec / 60))

# -------------------- manifest + skipped combos --------------------
manifest <- list(
  created_at      = as.character(Sys.time()),
  script          = "Run_D2_HighRho_Sims.R",

...
# ---- file prefix name ----
run_prefix <- "results/D2/D2_t_n1000_it1000_seed314_20260606_091405"
# reads: paste0(run_prefix, "_agg.csv")

# ---- output ----
out_dir  <- "figures"
out_stem <- "Fig_D2_t_Robust"
...
# ---- panel (A): mean coef +/- meanSD (not a CI band) ----
p_coef <- ggplot(dat, aes(x = f, y = beta, color = model, shape = model)) +
  ...
  labs(
    ...
    x = expression(pi[V]),
    # x = expression(paste("Share of Corr(A, ", N, ") from V (", pi[V],")")), 
...
DrawCombined <- function() {
  ...
  grid::grid.text("Noise: t (df=3), Robust SE", vp = vp(2, 1:2), y = unit(0.85, "npc"))

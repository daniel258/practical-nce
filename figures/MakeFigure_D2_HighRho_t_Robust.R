# MakeFigure_D2_HighRho_t_Robust.R
...
# Both shown for Model 2 (Y~A+N) and Model 4 (Y~A+N+V)
#
# Notes:
...
# ---- file prefix name ----
run_prefix <- "results/D2/D2_highRho_unequalA2_t_n1000_it1000_seed314_20260606_094144"
...
dat <- rbind(d2, d4)
dat$model <- factor(dat$model, levels = c("Y ~ A + N", "Y ~ A + N + V"))
...
p_nce <- ggplot(dat2, aes(x = f, y = beta, color = model, shape = model, group = model)) +
  ...
  labs(caption = "Noise: t (df=3), Robust SE") +
...
pdf(file.path(out_dir, paste0(out_stem, "_NCE_t_Robust.pdf")), width = width_in, height = height_in)
print(p_nce)
dev.off()
...
pdf(file.path(out_dir, paste0(out_stem, "_Power_t_Robust.pdf")), width = width_in, height = height_in)
print(p_pow)
dev.off()
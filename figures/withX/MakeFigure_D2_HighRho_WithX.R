# MakeFigure_D2_HighRho_WithX.R
# Design D2 (high rho_total), faceted by a1 (rows) and rho_total (cols).
# Outputs two files:
#   1) Mean NCE coefficient +/-  SD
#   2) Power (reject proportion) for H0: beta_N = 0
# Both shown for Model 2 (Y~A+N + X1 + X2) and Model 4 (Y~A+N+V + X1 + X2)
#
# Notes:
...
# ---- file prefix name ----
run_prefix <- "results/withX/D2/D2_highRho_withX_norm_n1000_it1000_seed314_20260605_203525"

out_dir  <- "figures/withX"
...
# Drop rho panels that are entirely infeasible (no rows in agg)
Rho_full <- Rho_full[Rho_full %in% sort(unique(agg$rho_plot))]
...
rho_labels  <- paste0("Corr(A,N)==", rho_levels)
...
agg$rho_label <- paste0("Corr(A,N)==", sprintf("%.2f", agg$rho_plot))
...
d2 <- data.frame(
  ...
  f     = agg$f_plot,
  model = "Y ~ A + N + X1 + X2",
  beta  = agg$beta_N_fit2,
  se    = agg$se_N_fit2,
  power = agg$power_N_fit2
)
...
d4 <- data.frame(
  ...
  f     = agg$f_plot,
  model = "Y ~ A + N + V + X1 + X2",
  beta  = agg$beta_N_fit4,
  se    = agg$se_N_fit4,
  power = agg$power_N_fit4
)

dat <- rbind(d2, d4)
dat$model <- factor(dat$model, levels = c("Y ~ A + N + X1 + X2", "Y ~ A + N + V + X1 + X2"))
...
p_nce <- ggplot(dat2, aes(x = f, y = beta, color = model, shape = model, group = model)) +
  ...
  facet_grid(a1 ~ rho, labeller = labeller(a1 = label_parsed, rho = label_parsed)) +  labs(
    ...
    x = XlabExpr,
    y = expression(paste("NCE coefficient")),# (", hat(beta)[N], ")")),
    color = "Model:",
    shape = "Model:"
  ) +
  BaseTheme() +
  scale_color_discrete(labels = c(
    expression(Y %~% A + N + X[1] + X[2]),
    expression(Y %~% A + N + V + X[1] + X[2])
  )) +
...
pdf(file.path(out_dir, paste0(out_stem, "_NCE_WithX.pdf")), width = width_in, height = height_in)
print(p_nce)
dev.off()
...
pdf(file.path(out_dir, paste0(out_stem, "_Power_WithX.pdf")), width = width_in, height = height_in)
print(p_pow)
dev.off()
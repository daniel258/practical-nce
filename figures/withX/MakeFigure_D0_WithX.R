# MakeFigure_D0_WithX.R
# Requires: ggplot2
# NOTE: results are adjusted for measured covariates X1 (continuous) and X2 (binary).

# ---- file prefix name ----
run_prefix <- "results/withX/D0/D0_withX_norm_n1000_it1000_seed314_20260605_193814"
# Reads below: paste0(run_prefix, "_agg.csv")

# ---- output ----
out_dir  <- "figures/withX"
out_stem <- "Fig_D0_WithX"
...
p_coef <- ggplot(agg, aes(x = a1, y = beta_N_fit2, color = c1_f)) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 1.6) +
...
p_pow <- ggplot(agg, aes(x = a1, y = power_N_fit2, color = c1_f)) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 1.6) +
...
p_sd <- ggplot(agg, aes(x = a1, y = sd_beta_N_fit2, color = c1_f)) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 1.6) +

# MakeFigure_V_D2_VaryA1Rho_WithX.R
...
# ---- file prefix name ----
run_prefix <- "results/withX/withV/D2/withV_D2_varyA1Rho_withX_norm_n1000_it1000_seed314_20260303_160910"
...
agg$rho_facet <- factor(
  paste0("Corr(A,~tilde(A))==", sprintf("%.2f", agg$rho_target)),
  levels = paste0("Corr(A,~tilde(A))==", sprintf("%.2f", rho_levels))
)
...
dat <- rbind(d2, d4)
dat$model <- factor(dat$model, levels = c("Y ~ A + Atilde + X1 + X2", "Y ~ A + Atilde + V + X1 + X2"))
...
p_nce <- ggplot(dat2, aes(x = f, y = beta, color = model, shape = model, group = model)) +
  ...
  scale_color_discrete(labels = c(
    expression(Y %~% A + tilde(A) + X[1] + X[2]),
    expression(Y %~% A + tilde(A) + V + X[1] + X[2])
  )) +
